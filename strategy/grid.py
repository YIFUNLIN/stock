...
def trade(real_movement, initial_money, rsi_period, low_rsi, high_rsi, ema_period,cool_down_period, print_log):
    money = initial_money
    states_buy = []
    states_sell = []
    ...
    for i in range(len(real_movement)):
        if i - last_action_day > cool_down_period:  # 判斷冷卻期是否已經過去
